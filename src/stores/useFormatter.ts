  ...
  actions: {
    ...
    formatToken(
      ...
      if (token && token.amount && token?.denom) {
        ...
        if (amount === 0) {
          return withDenom ? `0 ${denom.substring(0, 10)}` : '0';
        }

        if (amount < 0.00000000000001) {
          return withDenom
            ? `<0.00000000000001 ${denom.substring(0, 10)}`
            : `${amount}`;
        }

        if (amount < 0.000001) {
          const amountExpanded = amount.toFixed(15);
          // remove trailing zeros
          const amountTrimmed = amountExpanded.replace(/0+$/, '');
          return withDenom
            ? `${amountTrimmed} ${denom.substring(0, 10)}`
            : amountTrimmed;
        }
        if (amount < 0.01) {
          fmt = '0.[000000]';
