...
const totalAmountByCategory = computed(() => {
  ...
  delegations.value?.forEach((x) => {
    sumDel += Number(format.formatTokenAmount(x.balance));
  });
  let sumRew = 0;
  rewards.value?.total?.forEach((x) => {
    sumRew += Number(format.formatTokenAmount(x));
  });
  let sumBal = 0;
  balances.value?.forEach((x) => {
    sumBal += Number(format.formatTokenAmount(x));
  });
  let sumUn = 0;
  ...
  <div v-if="account">
    ...
    <div class="bg-base-100 px-4 pt-3 pb-4 rounded mb-4 shadow">
      ...
      <div class="grid md:!grid-cols-3">
        ...
        {{ console.log('totalAmountByCategory', totalAmountByCategory) }}
                ...
                <div class="text-xs">
                  {{
                    format.calculatePercent(
                      Number(format.formatTokenAmount(balanceItem)),
                      totalAmount,
                    )
                  }}
                </div>
